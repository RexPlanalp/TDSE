...
def checkNorm():
    ...
    print("Norm of Initial State:",np.abs(initial_prod))
    print("Norm of Final State:",np.abs(final_prod))
    return
def probDisribution():
    with open('input.json', 'r') as file:
            input_par = json.load(file)
    ...
    for l in range(lmax+1):
        ...
        print(f"Norm of {l} block", np.abs(final_prod))


        prob_list.append(final_prod)
    print(np.sum(np.abs(prob_list)))

    plt.bar(range(lmax+1),np.abs(prob_list))
