...
if INITIALIZE:
    ...
    S = PETSc.Mat().createAIJ([len(wavefunction),len(wavefunction)],nnz =(2*order + 1))
    ...
    H_0 = PETSc.Mat().createAIJ([len(wavefunction),len(wavefunction)],nnz =(2*order + 1) )
    ...
    S_TILE = kronV4(ones,S_R,n_basis*(2*order + 1))
...
def photoAngularV1(E_range,dphi,dtheta):
    ...
    for E in E_range:
        ...
        for theta_val in theta:
            ...
            spherical_vector = PETSc.Vec().createSeq(size=len(wavefunction))

            
            # Prepare arrays of indices and values
            indices = np.arange(len(wavefunction),dtype = "int32")
            ...
            spherical_vector.setValues(indices, values)
            ...
            x.pointwiseMult(x, spherical_vector)
