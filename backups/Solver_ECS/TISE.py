...
class tise:
    ...
    def _createH_l(self,basisInstance,l):
        def _H_element(x,i,j):
            return basis_funcs[i](x) * (-1/2) * basis_funcs[j](x,2) + basis_funcs[i](x) * basis_funcs[j](x) * l*(l+1)/(2*np.sqrt(x**4 + 1E-25 )) + basis_funcs[i](x) * basis_funcs[j](x)* self.pot_func(x)
    ...
    def _createH_l_ECS(self,basisInstance,l):
        def _H_element(x,i,j):
            return basis_funcs[i](x,1) * (1/2) * basis_funcs[j](x,1)/self.q(x) + basis_funcs[i](x) * basis_funcs[j](x) * l*(l+1)/(2*np.sqrt(self.R(x)**4 + 1E-25 ))*self.q(x) + basis_funcs[i](x) * basis_funcs[j](x)* (-1/np.sqrt(self.R(x)**2 + 1E-25))*self.q(x)
        #def _H_element(x,i,j):
            #return basis_funcs[i](x,1) * (1/2) * basis_funcs[j](x,1)/(self.q(x)**2) + basis_funcs[i](x) * basis_funcs[j](x) * l*(l+1)/(2*np.sqrt(self.R(x)**4 + 1E-25 )) + basis_funcs[i](x) * basis_funcs[j](x)* (-1/np.sqrt(self.R(x)**2 + 1E-25))
        n_basis = basisInstance.n_basis
        basis_funcs = basisInstance.basis_funcs
        order = basisInstance.order
    ...
    def createAllH_ECS(self,basisInstance):
        for l in range(self.lmax+1):
            self._createH_l_ECS(basisInstance,l)
        return None