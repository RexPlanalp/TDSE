...
if ENERGY: 
    ...
    E_range = np.arange(input_par["PES"][1],input_par["PES"][2]+2*gamma,2*gamma)
...
def photoSpectrum(E_range,theta_range,phi_range,PAD,PES):
    PES_vals = []
    E_vals = []
    theta_vals = []
    phi_vals = []
    ...
    for E in E_range:
        in_PES = input_par["PES"][1]<=E<=input_par["PES"][2]
        in_PAD = input_par["PAD"][1]<= E <= input_par["PAD"][2]
        print(f"Energy:{E},PES:{in_PES},PAD:{in_PAD}")
...
start = time.time()
photoSpectrum(E_range,theta_range,phi_range,PAD,PES)
end = time.time()
